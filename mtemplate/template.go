...
// Error contains information about an error returned during parsing and execution.
// Users may extract the information and reformat if they desire.
type Error struct {
	Line int
	Msg  string
}
...
func (t *Template) ParseFile(filename string) (err error) {
	b, err := TemplateSourceReader(filename)
	if err != nil {
		return err
	}
...
// ClearFromCache removes a parsed template for the given filename from the cache,
// casuing it to be reparsed on the next call.
...
// FileReader gives a function signature that can be used to read data identified
// by a file name without knowing the source of the data.
type FileReader func(string) ([]byte, error)

// TemplateSourceReader is a package property that can be given a value to allow the
// calling code to specify its own source of template data. By default, this is the
// file system, as provided by ioutil.ReadFile
var TemplateSourceReader FileReader = ioutil.ReadFile

// Cache can be set by the calling code to turn on or off the caching of parsed templates
var Cache bool

// CustomFormatters can be set by the calling code to a FormatterMap that is specific to its
// needs.
var CustomFormatters FormatterMap
